...
# قائمة الكلمات المفتاحية للتصيد (Blacklist)
PHISHING_KEYWORDS = ['login', 'bank', 'free-money', 'verify', 'account', 'secure-update', 'rewards', 'prize', 'iclod', 'apple-secure']
...
async def start(update, context):
    ...
    await update.message.reply_text("🦅 صقر الحماية جاهز، اختر الخدمة للفحص في بيئة آمنة:", reply_markup=InlineKeyboardMarkup(keyboard))
    return MODE
...
async def button_handler(update, context):
    ...
    await query.edit_message_text(f"✅ تم اختيار الفحص لـ {context.user_data['mode']}. أرسل الرابط أو الملف الآن:")
    return MODE
